	...
	text = function(bufnr)
		if git_comps.is_git(bufnr) then
			return {
				...
				{ git_comps.diff_removed({ format = '  %s', show_zero = true }), 'yellow' },
...
basic.lsp_progress = {
	...
	text = function(bufnr)
		return {
			{ #vim.lsp.buf_get_clients() > 0 and require("lsp").lsp_progress(false) or "", 'yellow' },
		}
	end,
}
...
local default = {
	...
	active = {
		...
		{ ' ', hl_list.Black },
		basic.lsp_name,
		basic.lsp_client_offset_encoding,
		basic.lsp_progress,
		basic.lsp_diagnos,
		basic.divider,
		basic.square_mode,
